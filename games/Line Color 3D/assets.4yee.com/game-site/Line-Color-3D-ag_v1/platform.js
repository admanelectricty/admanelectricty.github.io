!function(){const TAG="GamemonetizeAdsInstance_AD_WEB";class GamemonetizeAdsInstance{constructor(){this.isInited=false;this.appName="";}
adsAsyncInit(appName,type,gamedistributionAppId){this.route=["https://www.yad.com/","https://www.yiv.com/","https://www.babygames.com/","https://www.bestgames.com/","https://cargames.com/","https://www.yad.com/","https://www.yad.com/","https://www.yad.com/"][type];this.appName=appName;this.getForgames();return new Promise((resolve,reject)=>{window["SDK_OPTIONS"]={gameId:gamedistributionAppId,onEvent:(event)=>{console.log("event.name ====",event);switch(event.name){case "SDK_GAME_PAUSE":window.WebAudioEngine&&(window.WebAudioEngine.muted=true);break;case "SDK_GAME_START":if(event.status==="success"){if(this.onSuccess){this.onSuccess(false);this.onSuccess=null;}
this.onComplete();}
break;case "SDK_READY":resolve(this.isInited);break;default:break;}}};this.init().then((isInited)=>{this.isInited=isInited;});});}
init(){return new Promise((resolve,reject)=>{var t=this;var ads=document.getElementById(TAG);if(ads&&!this.isInited){ads=null;}
if(!ads){function onLoaded(){resolve(true);}
function onError(e){console.log("onError",e);reject(false);}
const library=document.createElement("script");library.onload=onLoaded.bind(this);library.onerror=onError.bind(this);library.type="text/javascript";library.async=false;library.src="https://api.gamemonetize.com/sdk.js";library.id=TAG;document.head.appendChild(library);}else{resolve(true);}});}
onComplete(){}
request(){return new Promise((resolve,reject)=>{if(!this.isInited){resolve(false);return;}
this.onSuccess=resolve;window["sdk"].showBanner();});}
showInterstitial(success){this.request().then(()=>{success&&success()});}
showReward(success,f){this.request().then(()=>{success&&success()});}
getForgames(){const xhr=new XMLHttpRequest();xhr.open("GET",this.route+"forgame/games.json",true);xhr.setRequestHeader("Content-Type","application/x-www-form-urlencoded; charset=UTF-8");xhr.responseType="text";xhr.onerror=function(e){};xhr.onabort=function(e){};xhr.onprogress=function(e){};xhr.onload=(e)=>{var status=xhr.status!==undefined?xhr.status:200;if(status===200||status===204||status===0){this.forgames=JSON.parse(xhr.responseText);}else{}};xhr.send();}
navigate(screenName,buttonName,gameId){gameId=gameId||"";let domain=document.referrer;var url=this.route;if(typeof gameId==='undefined'||gameId==""||gameId=="undefined"){}else{url=url+"?pic="+gameId;}
if(typeof domain==='undefined'||domain==""||domain=="undefined"){domain="unknown";}else{domain=domain.split('/')[2];}
if(url.indexOf("?")>-1){url=url+"&";}else{url=url+"?";}
url=url+"utm_source="+domain+"&utm_medium="+screenName+"-"+buttonName+
"&utm_campaign=game-"+this.appName;try{if(window.open(url)){}else{}}catch(error){}}}
class WebAudioEngine{constructor(){this.beEnabled=false;this.isMuted=false;this.bePauseSound=false;this.bePauseMusic=false;this.tryToResumeIntervalId=-1;this.isVisibilityMuted=false;this.adShowing=false;}
...
set pause(b){this.pauseSound=b;this.pauseMusic=b;}
...
return;this.stop(url);if(sound.buffer){try{instance.playBuffer(this.context.currentTime,sound.buffer);instance.source.loop=loop;}catch(e){console.error("playBuffer error. Exception: "+e);}}}else{this.downloadArrayBuffer(url,()=>{this.play(url,loop);});}}
...
onNavigate_(){if(this.canNavigateActive_){window.adsGameCon.moreGame();}
...
showInterstitial(complete){window.adsGameCon.showAd({beforeShowAd:()=>{window.WebAudioEngine.adShowing=true;this.onblur();Laya.timer.scale=0;Laya.stage.renderingEnabled=false
Laya.updateTimer&&Laya.updateTimer.pause()
Laya.physicsTimer&&Laya.physicsTimer.pause()},afterShowAd:()=>{window.focus();this.onfocus();window.WebAudioEngine.adShowing=false;Laya.timer.scale=1;Laya.stage.renderingEnabled=true
Laya.updateTimer&&Laya.updateTimer.resume()
Laya.physicsTimer&&Laya.physicsTimer.resume()
complete&&complete();}});}
showReward(success,failure){window.adsGameCon.showAdOfEvent({onRewardBeforeBreak:()=>{window.WebAudioEngine.adShowing=true;this.onblur();Laya.timer.scale=0;Laya.stage.renderingEnabled=false
Laya.updateTimer&&Laya.updateTimer.pause()
Laya.physicsTimer&&Laya.physicsTimer.pause()},onRewardAfterBreak:()=>{window.focus();this.onfocus();window.WebAudioEngine.adShowing=false;Laya.timer.scale=1;Laya.stage.renderingEnabled=true
Laya.updateTimer&&Laya.updateTimer.resume()
Laya.physicsTimer&&Laya.physicsTimer.resume()},onRewardComplete:()=>{success&&success();success=null;},onRewardDismissed:()=>{if(failure){failure();failure=null;}
this.prompt("Please watch the ad completely, so that you can claim your reward.");}});}
initList(appList){appList.renderHandler=new Laya.Handler(appList,function(e){e.offAll(Laya.Event.MOUSE_DOWN);e.on(Laya.Event.MOUSE_DOWN,e,()=>{platform.getInstance().navigate("GAME","MORE",e.dataSource.id)});})
appList.array=platform.getInstance().getForgames();}
prompt(msg,duration){if(!this.prompt_){this.prompt_=document.createElement('div');this.prompt_.style.cssText="font-family:siyuan;max-width:80%;min-width:320px;padding:10px 10px 10px 10px;min-height:40px;color: rgb(255, 255, 255);line-height: 20px;text-align:center;border-radius: 4px;position: fixed;top: 40%;left: 50%;transform: translate(-50%, -50%);z-index: 999999;background: rgba(0, 0, 0,.7);font-size: 16px;";document.body.appendChild(this.prompt_);}
this.prompt_.innerHTML=msg;duration=isNaN(duration)?2000:duration;this.prompt_.style.display="inline";this.prompt_.style.opacity='1';setTimeout(function(){var d=0.5;this.prompt_.style.webkitTransition='-webkit-transform '+d+'s ease-in, opacity '+d+
's ease-in';this.prompt_.style.opacity='0';this.prompt_.style.display="none";}.bind(this),duration);}
getForgames(){return null;}
createLogo(){const yad=new Laya.Image();yad.skin="yad.png";yad.zOrder=2e5;Laya.stage.addChild(yad);return yad;}
yadstartup(name,complete){if(!this.needStartUp){complete&&complete();}
if(this.initialized_)return;this.createNoVideo();this.createLoading();window.WebAudioEngine.init().then(()=>{Laya.SoundManager.playMusic=function(url){window.WebAudioEngine&&window.WebAudioEngine.playMusic(url);}
Laya.SoundManager.playSound=function(url){window.WebAudioEngine&&window.WebAudioEngine.playSound(url);}
Laya.SoundManager.stopMusic=function(url){window.WebAudioEngine&&window.WebAudioEngine.stopMusic();}})
this.initialized_=true;Laya.loader.load("cnf.json",Laya.Handler.create(this,(res)=>{this.needStartUp=false;complete&&complete();}))}
showBanner(){}
hideBanner(){}
showSplash(){}
hideSplash(){}
cargamesstartup(name,complete){if(!this.needStartUp){complete&&complete();}
if(this.initialized_)return;this.createNoVideo();this.createLoading();window.WebAudioEngine.init().then(()=>{Laya.SoundManager.playMusic=function(url){window.WebAudioEngine&&window.WebAudioEngine.playMusic(url);}
Laya.SoundManager.playSound=function(url){window.WebAudioEngine&&window.WebAudioEngine.playSound(url);}
Laya.SoundManager.stopMusic=function(url){window.WebAudioEngine&&window.WebAudioEngine.stopMusic();}})
this.initialized_=true;Laya.loader.load("cnf.json",Laya.Handler.create(this,(res)=>{this.needStartUp=false;complete&&complete();}))}
